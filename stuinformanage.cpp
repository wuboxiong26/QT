...
void stuinformanage::on_infor_see_inforpushButton_clicked()  //管理端查看所有学生信息
{
    QSqlDatabase db;
    if(QSqlDatabase::contains("qt_sql_default_connection"))
        db = QSqlDatabase::database("qt_sql_default_connection");
    ...
    while(query.next())
    {
        ...
        Globel::list_all_student.append(q);
    }


    for(int i=0;i<Globel::list_all_student.size();i++){
        qDebug()<<Globel::list_all_student[i];
    }
